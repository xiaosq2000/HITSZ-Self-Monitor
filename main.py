...
with open("profiles.json", "r") as profiles:
    personal_info = json.load(profiles)

options = Options()
options.headless = True
options.add_argument("--log-level=3")
driver = webdriver.Chrome(ChromeDriverManager(path='.', print_first_line=False).install(), options=options)
...
print("输入学号")
driver.find_element_by_xpath(
    '/html/body/div/div[2]/div/form/div/div[2]/ul/li[1]/input[1]').send_keys(personal_info["id"])
print("输入密码")
driver.find_element_by_xpath(
    '/html/body/div/div[2]/div/form/div/div[2]/ul/li[1]/input[2]').send_keys(personal_info["password"])
driver.find_element_by_xpath(
...
if flag == '已提交':
    ...
    picker_indicator = driver.find_element_by_xpath(
    ...
    while index < personal_info["current_status"]:
        action.move_to_element(picker_indicator)
        action.drag_and_drop_by_offset(picker_indicator, 0, -34)
        index += 1
